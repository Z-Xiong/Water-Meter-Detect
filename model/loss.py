...
class YOLOLoss(nn.Module):
    ...
    def compute_iou(self, box1, box2):
        '''计算两个box集合的iou，每个box是[x1, y1, x2, y2]
        Args:
        ...
        rb = torch.min(
            box1[:, 2:].unsuqeeze(1).expand(N, M, 2),  # [N, 2] -> [N, 1, 2] -> [N, M, 2]
            box2[:, 2:].unsuqeeze(0).expand(N, M, 2),  # [M, 2] -> [1, M, 2] -> [N, M, 2]
        )
        ...
        area1 = area1.unsuqeeze(1).expand_as(inter)  # [N, ] -> [N, 1] -> [N, M]
        area2 = area2.unsuqeeze(0).exppand_as(inter)  # [M, ] -> [1, M] -> [N, M]
...
def yolo_loss(pred_tensor, target_tensor):
    ...
    return criterion(pred_tensor, target_tensor)
